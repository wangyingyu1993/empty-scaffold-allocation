#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Generate standard Bemis-Murcko scaffold splits.
...
def summarize_split(df: pd.DataFrame) -> Dict[str, float | int]:
    n = len(df)
    if n == 0:
        return {
            "molecule_count": 0,
...
def assign_groups_to_parts(
    ...
    # Put the k largest groups into different parts first.
    for part_id, (_, rowids) in enumerate(items[:kfolds]):
        for rid in rowids:
            rowid_to_part[rid] = part_id
        part_sizes[part_id] += len(rowids)

    # Greedily add remaining groups to the currently smallest part.
    ...
    # Randomly relabel part IDs so the largest groups are not always early parts.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate standard Bemis-Murcko scaffold split folds.")

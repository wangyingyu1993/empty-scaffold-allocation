...
def _rng(dataset_name: str):
    return random.Random((hash(dataset_name) + RANDOM_SEED) & 0xFFFFFFFF)
...
保留的数据集/任务：共 **{summary['n_datasets_total']}** 个，其中 MoleculeNet **{summary['n_datasets_moleculenet']}** 个，ADMET **{summary['n_datasets_admet']}** 个；分类 **{summary['n_datasets_classification']}** 个，回归 **{summary['n_datasets_regression']}** 个。

## Structural-space summary
- 无骨架分子 median mean pairwise similarity: **{summary['median_no_pairwise']:.3f}**
- 全部有骨架分子混合后的 median mean pairwise similarity: **{summary['median_hasmixed_pairwise']:.3f}**
- 真实 scaffold 类内 median mean pairwise similarity: **{summary['median_true_scaffold_pairwise']:.3f}**
- 无骨架分子 median cohesion Gini: **{summary['median_no_gini']:.3f}**
- 全部有骨架分子混合后的 median cohesion Gini: **{summary['median_hasmixed_gini']:.3f}**
- 真实 scaffold 类内 median cohesion Gini: **{summary['median_true_scaffold_gini']:.3f}**
...
- median no→scaffold NN similarity: **{summary['median_no_to_scaffold_nn']:.3f}**

## Label-space summary
- 回归 targets: **{summary['n_regression_targets']}**
- 其中 |Hedges g| ≥ 0.5 的回归 targets: **{summary['n_regression_abs_g_ge_0p5']}**
- 其中 |Hedges g| ≥ 0.8 的回归 targets: **{summary['n_regression_abs_g_ge_0p8']}**
- 分类 targets: **{summary['n_classification_targets']}**
- 其中 |Δ positive rate| ≥ 0.10 的分类 targets: **{summary['n_classification_prevdiff_ge_0p10']}**
- 其中 |Δ positive rate| ≥ 0.20 的分类 targets: **{summary['n_classification_prevdiff_ge_0p20']}**

## Top shifted targets
- 回归（按 |Hedges g| + KS 排序）: {", ".join(summary['top_regression_shift_targets']) if summary['top_regression_shift_targets'] else "NA"}
- 分类（按 |Δ positive rate| + JSD 排序）: {", ".join(summary['top_classification_shift_targets']) if summary['top_classification_shift_targets'] else "NA"}
"""
